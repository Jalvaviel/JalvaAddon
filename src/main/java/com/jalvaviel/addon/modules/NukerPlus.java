...
public class NukerPlus extends Module {
    ...
    private final Setting<meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape> shape = sgGeneral.add(new EnumSetting.Builder<meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape>()
        .name("shape")
        .description("The shape of nuking algorithm.")
        .defaultValue(meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.Sphere)
        .build()
    );

    private final Setting<meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode> mode = sgGeneral.add(new EnumSetting.Builder<meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode>()
        .name("mode")
        .description("The way the blocks are broken.")
        .defaultValue(meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode.Flatten)
        .build()
    );
    ...
    private final Setting<Double> range = sgGeneral.add(new DoubleSetting.Builder()
        ...
        .defaultValue(4)
        .min(0)
        .visible(() -> shape.get() != meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.Cube)
        .build()
    );
    ...
    private final Setting<meteordevelopment.meteorclient.systems.modules.world.Nuker.SortMode> sortMode = sgGeneral.add(new EnumSetting.Builder<meteordevelopment.meteorclient.systems.modules.world.Nuker.SortMode>()
        .name("sort-mode")
        .description("The blocks you want to mine first.")
        .defaultValue(meteordevelopment.meteorclient.systems.modules.world.Nuker.SortMode.Closest)
        .build()
    );
    ...
    private final Setting<meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode> listMode = sgWhitelist.add(new EnumSetting.Builder<meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode>()
        .name("list-mode")
        .description("Selection mode.")
        .defaultValue(meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode.Blacklist)
        .build()
    );
    ...
    private final Setting<List<Block>> blacklist = sgWhitelist.add(new BlockListSetting.Builder()
        ...
        .visible(() -> listMode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode.Blacklist)
        .build()
    );
    ...
    private final Setting<List<Block>> whitelist = sgWhitelist.add(new BlockListSetting.Builder()
        ...
        .visible(() -> listMode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode.Whitelist)
        .build()
    );
    ...
    public NukerPlus() {
        super(Categories.World, "nukerplus", "Breaks blocks around you.");
    }
    ...
    private void onRender(Render3DEvent event) {
        if (enableRenderBounding.get()){
            ...
            if (shape.get() != meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.Sphere && mode.get() != meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode.Smash) {
    ...
    private void onTickPre(TickEvent.Pre event) {
        ...
        if (shape.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.UniformCube) range.set((double) Math.round(range.get()));
        ...
        if (shape.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.UniformCube) {
        ...
        if (mode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode.Flatten){
        ...
        BlockIterator.register(Math.max((int) Math.ceil(range.get()+1), maxh), Math.max((int) Math.ceil(range.get()), maxv), (blockPos, blockState) -> {
            ...
            if (!BlockUtils.canBreak(blockPos, blockState)
                || (toofarSphere && shape.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.Sphere)
                || (toofarUniformCube && shape.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.UniformCube)
                || (toofarCube && shape.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Shape.Cube))
                return;

            // Flatten
            if (mode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode.Flatten && blockPos.getY() < Math.floor(mc.player.getY())) return;

            // Smash
            if (mode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.Mode.Smash && blockState.getHardness(mc.world, blockPos) != 0) return;

            // Check whitelist or blacklist
            if (listMode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode.Whitelist && !whitelist.get().contains(blockState.getBlock())) return;
            if (listMode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.ListMode.Blacklist && blacklist.get().contains(blockState.getBlock())) return;
        ...
        BlockIterator.after(() -> {
            // Sort blocks
            if (sortMode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.SortMode.TopDown)
                blocks.sort(Comparator.comparingDouble(value -> -1*value.getY()));
            else if (sortMode.get() != meteordevelopment.meteorclient.systems.modules.world.Nuker.SortMode.None)
                blocks.sort(Comparator.comparingDouble(value -> Utils.squaredDistance(pX, pY, pZ, value.getX() + 0.5, value.getY() + 0.5, value.getZ() + 0.5) * (sortMode.get() == meteordevelopment.meteorclient.systems.modules.world.Nuker.SortMode.Closest ? 1 : -1)));
